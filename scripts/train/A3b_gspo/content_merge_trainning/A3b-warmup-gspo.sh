...
# ===================================== Model & Experiment =====================================
actor_model_path="/home/jovyan/work/lns/Task_SFT/merged_model/sft56_ep3_ms-0-3-.75"  #SFT56 merge 后的模型
critic_model_path="/mnt/public_data/Qwen/Qwen3-14B"  # 使用较小的模型作为 Critic

backend=fsdp2  # fsdp, fsdp2, megatron
project_name=kowledge_analysis_A3b
experiment_name=2-3-warmup-v10-$backend
...
actor_lr=0     # 增加学习率，让 Actor 能够学习
actor_grad_clip=0.3  # 放宽梯度裁剪
critic_lr=1e-5    # 降低 Critic 学习率，防止 Discriminator 过拟合
critic_grad_clip=0.5  # 官方有梯度裁剪
gae_gamma=1.0
gae_lam=0.95
critic_warmup=20  # 减少 warmup，防止 Discriminator 过拟合

# ===================================== Data/Model =====================================
train_files=/home/jovyan/JQ/gad_gspo_B300/data/trainning_dataset/subject_1-29/merged/merge-1-29.parquet
test_files=/home/jovyan/JQ/gad_gspo_B300/data/trainning_dataset/subject_1-29/merged/merge-1-29.parquet

max_prompt_length=3900    # core_content 数据集，与 GAD 一致
max_response_length=550   # core_content P99=174，256 足够
max_model_len=5000   # 与 GAD 一致
train_batch_size=256      # 修改：减小 batch size，增加训练步数
val_batch_size=128
...
n_resp_per_prompt=4  # 增加到 8，与官方一致
temperature=0.6  # 增加多样性，让 GRPO 能区分不同 response，防止 Discriminator 过拟合
