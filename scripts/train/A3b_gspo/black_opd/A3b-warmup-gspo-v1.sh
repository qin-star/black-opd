...
backend=fsdp2  # 使用FSDP2: -7%内存, +1.5%吞吐量, 更好的MoE支持
...
kl_coef=0.001
use_kl_loss=True
kl_loss_coef=0.3    # 适度KL约束，平衡探索与稳定性
...
critic_lr=5e-6    # 提高Critic学习率，加速学习（当前d_acc仅5%）
critic_grad_clip=1.0  # 放宽梯度裁剪，当前grad_norm=14过高被截断
gae_gamma=1.0
gae_lam=0.95
critic_warmup=20  # 增加warmup，让Critic充分学习（当前d_acc仅5%）

# ===================================== Data/Model =====================================
train_files=/home/jovyan/JQ/gad_gspo_B300/scripts/data_process/trainning_data/black_opd/lmsys_gpt5_chat_4k_filtered_train_cleaned.parquet
test_files=/home/jovyan/JQ/gad_gspo_B300/scripts/data_process/trainning_data/black_opd/lmsys_gpt5_chat_4k_filtered_test_cleaned.parquet

max_prompt_length=650   # core_content 数据集，与 GAD 一致
max_response_length=1280    # warmup阶段缩短，减少计算量
train_batch_size=128      # 增加batch size，提升GPU利用率和吞吐量
val_batch_size=128
ppo_mini_batch_size=64    # 相应增加，保持Off-policy=4倍

# ===================================== Training =====================================
# B300 268GB 显存，优化token容量
# 计算: train_batch_size * n_resp * (max_prompt + max_response) / n_gpus
# = 256 * 4 * (650 + 1280) / 8 = 247,040 tokens/gpu (理论最大)
# 设置为略高于此值，留有余量
actor_max_token_len_per_gpu=65536  # 96K tokens/gpu，匹配更大的batch和序列长度
...
rollout_name=vllm
infer_tp=4  # tensor并行度
gpu_memory_utilization=0.7  # GPU利用率
# max_model_len = max_prompt_length + max_response_length + 余量 = 650 + 1280 + 70 = 2000
max_model_len=2000   # 匹配实际序列长度需求
n_resp_per_prompt=2  # 每个prompt生成4个response warmup阶段降低为2，提高速度
